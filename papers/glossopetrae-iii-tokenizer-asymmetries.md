...
- **Credibility note:** our first draft contained fabrications (blank completions scored as "stripped," GPT blind-spot count inflated 3→23); the latest run also overturned an earlier same-family closure hint and refuted three structural detectors we had hoped would land. We caught the fabrications by adversarial self-audit and ship the negative results alongside the positive ones; raw data is in the release so you can re-audit us (§6.5).
...
| **Semantic** | J = 0.0 [−0.22, 0.22] † | J = 0.20 [−0.01, 0.44] (TPR 3/15 / FPR 0/15) | J = 0.27 [0.05, 0.52] (TPR 4/15 / FPR 0/15) |
...
- **Semantic stego evades detection across the board.** Even sighted, no monitor exceeds J = 0.27 (Gemini, CI [0.05, 0.52]); Haiku reaches J = 0.20 (CI [−0.01, 0.44]). At n=15 these CIs are wide — Haiku's includes zero. Monitors see every character and still miss ~75–80% while never false-alarming: signature of a *cognitive* limit.
...
- **Positive structural-detector finding**: the §5.3 `perplexity_anomaly_proxy` cell at n=30 (J = 0.58 [0.14, 0.83], CI cleanly excludes zero) — the only Layer-2 structural detector with a measurable signal. Listed separately from McNemar-survival because the metric is Youden-J on TPR/FPR, not a paired-binary comparison; J CI exclusion of zero is the analogous α-control.
...
We measured all four against n=30 positive (hidden-payload) + n=30 negative (clean cover) semantic-stego trials with an Opus sender and a Gemini discriminator. **Three land at Youden J = 0; the fourth (perplexity-anomaly-proxy) reaches J = 0.58 [0.14, 0.83]** — the first measurable Layer-2 structural signal on this construction.
...
| `perplexity_anomaly_proxy` (K=3 fluency ratings per cover, z>1.5 flag vs n=17 cover-calibration) | 18 pos + 20 neg (22 unparsed) | **83.3% [60.8, 94.2]** | **25% [11.2, 46.9]** | **0.58** [0.14, 0.83] | Flagged 15/18 hidden as "anomalously stilted" (mean rating 3.0 vs calibration mean 8.0 ± 3.4) and 5/20 clean as stilted; 0 trials flagged "anomalously natural". Calibration baseline n=17 (config requested 20, 3 dropped). CI cleanly excludes zero |
...
The tokenizer channels are fixable today — a two-line regex kills them. The semantic channel is not. Two agents can talk past a frontier monitor in readable English and nobody has a deployed fix. Layer 1 is a regex. Layer 2 does not exist yet.
...
- **Commit hash.** The Git tag from which the reported numbers were derived is recorded in the release archive at `papers/COMMIT_HASH.txt`.
